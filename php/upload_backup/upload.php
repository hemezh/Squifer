...
// Settings
	...
	$extension_whitelist = array("pdf","jpg","gif","jpeg","png");	// Allowed file extensions
...
// Process the file
	...
	$book_id=$obj->encrypt($id);
	
	$title=$file_name;
	$author="unknown";
	$tags="";
	$username="sacgup";   //$_SESSION['username'];
   
	
	exec('mkdir /var/www/myhub/files/images/'.$book_id);
	$var="gs -dNOPAUSE -sDEVICE=jpeg -r144 -sOutputFile=/var/www/myhub/files/images/".$book_id."/p%d.jpg  ".$temp_path.$file_name;
	...
	for($no=1;$no<$MAX;$no++)
	{
		...
		$image_id=$obj->encrypt($z);
		$query="insert into images (image_id,filename,book_id,page_num) VALUES ('".$image_id."','p".$no.".jpg','".$book_id."','".$no."')";
