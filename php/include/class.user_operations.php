...
class user_operations extends book {
	...
	public function showUserBooks($username)

	{

		$query="SELECT * FROM books where username=\"$username\" order by time desc";
...

		{
			...
			$books[$row['book_id']]= $this->fetchBookDetail($row['book_id'] );

		} 

		$this->print_rows($books,"mydocs");
	...
	echo "<div class=\"mainPane\" ><table cellspacing=\"0\"  class=\"table table-striped table-condensed\">";	
		...
		if(count($records) >= 1)
  		...
  		{					
						...
						echo '<th width="500">Title</th>';

						echo '<th width="238">Author</th>';

						if($value!="bookmark")

						echo '<th width="418">Tags</th>';
						...
						//echo '<td>'.$details.'</td>'; 
						$title=$this->shortenName($title,50);
...

  		{
				 ...
				 //echo '<tr><td>&nbsp;</td></tr>';
				  ...
				  case "favourite":
				  		...
				  		echo 'You don\'t have any documents in you collection.';

				  		break;
				  	case "mydocs":

				  		echo 'You have not uploaded any documents yet..';
