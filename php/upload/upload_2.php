...
// Settings
	...
	$temp_path = "/var/www/myhub/files/tmp/";	
	//$temp_path = getcwd() . "/../../../../myhub/temp/";		// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
	$save_path = "/var/www/myhub/files/pdf/";
...
/* Validate that we won't over-write an existing file
	if (file_exists($tmp_path . $file_name)) {
...
// Validate file contents (extension and mime-type can't be trusted)
	/*
		...
		Therefore, no sample code will be provided here.  Research the issue, decide how much security is
		 needed, and implement a solution that meets the needs.h
...
// Process the file
	...
	//echo $temp_path.$file_name;
	
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $temp_path.$file_name)) {
	...
	//$id=$obj->generateId("book");  
	$book_id=$obj->encrypt($file_name);
	echo $book_id;
	
	//echo "************************";
	
	
	//$_SESSION["file_name"]=$book_id;
	//print_r($_SESSION);
//	session_destroy();
	//print_r($_SESSION);
	
	
	$title=$obj->titleCleaner($file_name);
	/*
	$tmp_title=str_replace('-',' ',$file_name);
	$tmp_title=str_replace('_',' ',$tmp_title);
	$tmp_title=substr($tmp_title,0,strlen($tmp_title)-4);
	$title=ucwords($tmp_title);
	*/
	$author="unknown";
	$tags="";
	...
	exec('mkdir /var/www/myhub/files/images/'.$book_id);
	$command="gs -dNOPAUSE -sDEVICE=pngalpha -dUseCropBox -r144 -sOutputFile=/var/www/myhub/files/images/".$book_id."/p%d.png  ".$temp_path.$file_name;
	//exec(mv $temp_path.'/'.$file_name '/var/myhub/pdf'.$file_name);
	exec($command);
	
	
	
	$filecount="ls -l /var/www/myhub/files/images/".$book_id."/ | wc -l";
	exec($filecount,$MAX);
	$command="touch /var/www/myhub/files/images/".$book_id."/index.html";
	exec($command);
	//print_r($MAX);
	
	
	$query="insert into books (book_id,title,author,tags,username) VALUES ( '".$book_id."','".$title."','".$author."','".$tags."','".$username."')";
	$sql->process_query($query);
	//$obj->updateCounter("book"); 
