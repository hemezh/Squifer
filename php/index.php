...
session_start();
	...
	if( isset($_POST['id']) )
	{
		...
		{ 
			if(isset($_SESSION['username']))
			{
				$obj3->showUserBooks($_SESSION['username']);
			}
			else
			echo "<br><br><br><br><br><h1><center><font size=\"6\">Please Login to see your Documents.</font></center></h1>";
		}
		$val=substr($id,0,8);
		if($val=="Search::")
		{
			$str=substr($id,8);
			if(strlen($str)<4)
			echo "<br><br><br><br><br><h1><center><font size=\"6\">Please enter a larger string</font></center></h1>";
			$value=$obj->searchBooks($str);
		}
		
		if($id=="Popular")
		{
			$value=$obj->printPopular(20);
		}
